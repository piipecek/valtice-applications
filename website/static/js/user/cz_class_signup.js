...
function update_div_obou_trid(trida) {
    ...
    console.log(main_id, typeof main_id)
    console.log(secondary_id, typeof secondary_id)
    ...
    // barvicka
    let main_color_div = document.createElement("div")
    main_trida_div.appendChild(main_color_div)
    let secondary_color_div = document.createElement("div")
    secondary_trida_div.appendChild(secondary_color_div)

    if (trida["state_main"] === "enrolled") {
        main_color_div.classList.add("trida_enrolled")
    } else if (trida["state_main"] === "available") {
        main_color_div.classList.add("trida_available")
    } else {
        main_color_div.classList.add("trida_full")
    }

    if (trida["state_secondary"] === "enrolled") {
        secondary_color_div.classList.add("trida_enrolled")
    }
    else if (trida["state_secondary"] === "available") {
        secondary_color_div.classList.add("trida_available")
    } else {
        secondary_color_div.classList.add("trida_full")
    }
...
for (let trida of tridy_a_kapacity) {
    ...
    trida_div.addEventListener("click", function() {
        if (trida_div.dataset.state === "full") {
            ...
            if (confirm("Opravdu se chcete z třídy odhlisit?")) {
