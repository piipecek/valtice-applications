...
@org_api.route("/seznam_korepetitoru")
@require_role_system_name_on_current_user("organiser")
def seznam_korepetitoru():
    repetiteur_role = Role.get_by_system_name("repetiteur")
    return json.dumps(sorted([u.info_pro_seznam_lektoru() for u in User.get_all() if repetiteur_role in u.roles and u.tutor_this_year], key=lambda x: czech_sort.key(x["surname"])))
...
def seznam_vypnutych_lektoru():
    tutor_role = Role.get_by_system_name("tutor")
    repetuiteur_role = Role.get_by_system_name("repetiteur")
    return json.dumps(sorted([u.info_pro_seznam_lektoru() for u in User.get_all() if (tutor_role in u.roles or repetuiteur_role in u.roles) and not u.tutor_this_year], key=lambda x: czech_sort.key(x["surname"])))
